...
const LABEL = process.argv[2] ?? 'Test 3206864490'
...
async function main() {
  ...
  await shot(page, 'debug-post-login')
  ...
  // Poll for the QR image to actually render (component polls status every 2s)
  const qrImg = page.locator('img[alt="Scan with WhatsApp"]')
  await qrImg.waitFor({ state: 'visible', timeout: 20000 })
  await page.waitForTimeout(1000)

  await shot(page, 'settings-whatsapp-qr-new')
